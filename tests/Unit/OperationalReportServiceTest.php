...
class OperationalReportServiceTest extends TestCase
{
    ...
    public function test_build_marks_child_delivery_orders_as_backorders(): void
    {
        $sales = User::factory()->create(['role' => 'sales']);

        $parent = DeliveryOrder::create([
            'sales_user_id' => $sales->id,
            'customer_name' => 'Parent Order',
            'delivery_date' => now()->toDateString(),
            'status' => 'partial',
        ]);

        DeliveryOrderItem::create([
            'delivery_order_id' => $parent->id,
            'part_number' => 'P-BACK-1',
            'quantity' => 100,
            'fulfilled_quantity' => 60,
        ]);

        $backorder = DeliveryOrder::create([
            'parent_delivery_order_id' => $parent->id,
            'sales_user_id' => $sales->id,
            'customer_name' => 'Parent Order',
            'delivery_date' => now()->toDateString(),
            'status' => 'approved',
        ...
        $backorderRow = $data['fulfillmentRows']->firstWhere('order_id', $backorder->id);

        $this->assertNotNull($backorderRow);
        $this->assertSame('Backorder', $backorderRow['status']);
        $this->assertSame(1, (int) $data['backorderCount']);
    }

    public function test_build_filters_fulfillment_rows_to_backorders_only(): void
    {
        $sales = User::factory()->create(['role' => 'sales']);
        ...
        $fullOrder = DeliveryOrder::create([
            'sales_user_id' => $sales->id,
        ...
        DeliveryOrderItem::create([
            'delivery_order_id' => $fullOrder->id,
            'part_number' => 'P-FILTER-2',
            'quantity' => 20,
        ...
        $data = $service->build(new Request([
            'period' => 'all',
            'fulfillment_filter' => 'backorder',
        ]));

        $this->assertSame('backorder', $data['fulfillmentFilter']);
        $this->assertCount(1, $data['fulfillmentRows']);
        $this->assertSame($backorder->id, $data['fulfillmentRows']->first()['order_id']);
        $this->assertSame(0, $data['fulfillmentRate']);
        $this->assertSame(1, (int) $data['backorderCount']);
    }
